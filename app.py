...
app.config['BACKUP_DAYS_TO_KEEP'] = 30
app.config['AUTO_BACKUP_HOUR'] = 2  # 凌晨2点自动备份
app.config['AUTO_BACKUP_MINUTE'] = 0
...
def get_database_tables():
    ...
    tables = [row['Tables_in_' + app.config['MYSQL_DB']] for row in cur.fetchall()]
    cur.close()
    return tables


def export_table_to_sql(table_name, file_handle):
    """导出单表数据到SQL文件"""
    cur = mysql.connection.cursor()
    ...
    # 写入创建表语句
    file_handle.write(f"-- Table structure for table `{table_name}`\n")
    file_handle.write(f"DROP TABLE IF EXISTS `{table_name}`;\n")
    file_handle.write(create_table_sql + ";\n\n")
    ...
    if rows:
        ...
        # 写入数据
        file_handle.write(f"-- Dumping data for table `{table_name}`\n")
        file_handle.write(f"LOCK TABLES `{table_name}` WRITE;\n")
        ...
        file_handle.write("UNLOCK TABLES;\n\n")
...
def create_backup(backup_type='manual', description=''):
    ...
    try:
        ...
        backup_filename = f"{app.config['MYSQL_DB']}_{timestamp}_{backup_type}.sql"
        backup_path = os.path.join(app.config['BACKUP_DIR'], backup_filename)

        logger.info(f"开始创建备份: {backup_filename}，描述: {description}")

        with open(backup_path, 'w', encoding='utf-8') as f:
            # 写入备份信息
            f.write(f"-- Database Backup for {app.config['MYSQL_DB']}\n")
            f.write(f"-- Backup Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"-- Backup Type: {backup_type}\n")
            if description:
                f.write(f"-- Description: {description}\n")
            f.write(f"-- Generated by Student Dorm Management System\n")
            ...
            for table in tables:
                records = export_table_to_sql(table, f)
                total_records += records
                logger.info(f"表 {table} 导出完成，{records} 条记录")
            ...
            f.write(f"-- Database: {app.config['MYSQL_DB']}\n")
            f.write(f"-- Host: {app.config['MYSQL_HOST']}\n")
            f.write(f"-- ========================================\n")

        # 创建压缩版本
        zip_filename = backup_filename.replace('.sql', '.zip')
        zip_path = os.path.join(app.config['BACKUP_DIR'], zip_filename)
        ...
        cur = mysql.connection.cursor()
        cur.execute("""
            INSERT INTO backups (filename, filepath, backup_type, description, size, created_by)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (zip_filename, zip_path, backup_type, description,
              os.path.getsize(zip_path), session.get('user_id', 'system')))
        ...
        logger.info(f"备份创建成功: {zip_filename}，大小: {os.path.getsize(zip_path) / 1024:.1f} KB")
        return {
            'success': True,
...
def cleanup_old_backups():
    ...
    try:
        ...
        days_to_keep = app.config['BACKUP_DAYS_TO_KEEP']
        ...
        if deleted_files:
            ...
            cur.execute(f"DELETE FROM backups WHERE filename IN ({placeholders})", deleted_files)
            mysql.connection.commit()
            cur.close()

            logger.info(f"清理旧备份完成，删除 {len(deleted_files)} 个文件")
...
def restore_backup(backup_id):
    ...
    try:
        ...
        logger.info(f"开始恢复备份: {backup['filename']}")
        ...
        if not sql_file:
            return {'success': False, 'error': '备份文件中未找到SQL文件'}
        ...
        cur = mysql.connection.cursor()
        cur.execute("""
            INSERT INTO backup_restores (backup_id, restored_by, success_count, error_count)
            VALUES (%s, %s, %s, %s)
        """, (backup_id, session.get('user_id'), success_count, error_count))

        mysql.connection.commit()
        cur.close()

        logger.info(f"备份恢复完成: 成功 {success_count} 条，失败 {error_count} 条")
...
def auto_backup_job():
    ...
    with app.app_context():
        try:
            logger.info("开始自动备份...")
            result = create_backup(backup_type='auto', description='系统自动备份')

            if result['success']:
                logger.info(f"自动备份完成: {result['filename']}")
            else:
                logger.error(f"自动备份失败: {result.get('error', '未知错误')}")

        except Exception as e:
            logger.error(f"自动备份任务异常: {str(e)}")
...
def start_scheduler():
    ...
    try:
        # 每天指定时间执行自动备份
        schedule.every().day.at(f"{app.config['AUTO_BACKUP_HOUR']:02d}:{app.config['AUTO_BACKUP_MINUTE']:02d}").do(
            auto_backup_job)
        ...
        logger.info(
            f"备份调度器已启动，每天 {app.config['AUTO_BACKUP_HOUR']:02d}:{app.config['AUTO_BACKUP_MINUTE']:02d} 执行自动备份")

    except Exception as e:
        logger.error(f"启动备份调度器失败: {str(e)}")
...
def backup_management():
    """备份管理页面"""
    logger.info(f"访问备份管理页面 - 用户: {session['user_id']}")

    cur = mysql.connection.cursor()

    # 获取备份列表
    cur.execute("""
        SELECT b.*, u.realname as creator_name 
        FROM backups b 
    ...
    # 获取备份统计
    cur.execute("SELECT COUNT(*) as total FROM backups")
    total_backups = cur.fetchone()['total']

    cur.execute("SELECT SUM(size) as total_size FROM backups")
    total_size = cur.fetchone()['total_size'] or 0

    # 获取自动备份设置
    auto_backup_settings = {
        'hour': app.config['AUTO_BACKUP_HOUR'],
        'minute': app.config['AUTO_BACKUP_MINUTE'],
        'days_to_keep': app.config['BACKUP_DAYS_TO_KEEP']
    }
...
@admin_required
@log_operation("创建数据备份")
def api_create_backup():
    """创建数据备份API"""
    try:
        description = request.form.get('description', '手动备份')

        result = create_backup(backup_type='manual', description=description)

        if result['success']:
            return jsonify({
                'success': True,
                'message': f'备份创建成功: {result["filename"]}',
                'data': result
            })
        else:
            return jsonify({
                'success': False,
                'message': f'备份创建失败: {result.get("error", "未知错误")}'
            }), 500
...
def api_restore_backup(backup_id):
    ...
    try:
        ...
        if result['success']:
            return jsonify({
                'success': True,
                'message': f'备份恢复成功: {result["success_count"]} 条成功，{result["error_count"]} 条失败',
                'data': result
            })
        else:
            return jsonify({
                'success': False,
                'message': f'备份恢复失败: {result.get("error", "未知错误")}'
            }), 500
...
@app.route('/api/backup/delete/<int:backup_id>', methods=['POST'])
@admin_required
@log_operation("删除数据备份")
def api_delete_backup(backup_id):
    ...
    try:
        ...
        # 获取备份信息
        cur.execute("SELECT filename, filepath FROM backups WHERE backup_id = %s", (backup_id,))
        backup = cur.fetchone()
        ...
        logger.info(f"删除备份文件: {backup['filename']}")
        ...
        logger.error(f"删除备份API失败: {str(e)}")
        return jsonify({
            'success': False,
            'message': str(e)
        }), 500
...
@admin_required
@log_operation("修改备份设置")
def api_update_backup_settings():
    """修改备份设置API"""
    try:
        auto_hour = request.form.get('auto_hour', type=int)
        auto_minute = request.form.get('auto_minute', type=int)
        days_to_keep = request.form.get('days_to_keep', type=int)

        # 验证输入
        if auto_hour is not None and not (0 <= auto_hour <= 23):
            return jsonify({
                'success': False,
                'message': '自动备份小时必须在0-23之间'
            }), 400

        if auto_minute is not None and not (0 <= auto_minute <= 59):
            return jsonify({
                'success': False,
                'message': '自动备份分钟必须在0-59之间'
            }), 400

        if days_to_keep is not None and days_to_keep < 1:
            return jsonify({
                'success': False,
                'message': '备份保留天数必须大于0'
            }), 400
        ...
        if auto_hour is not None:
            app.config['AUTO_BACKUP_HOUR'] = auto_hour

        if auto_minute is not None:
            app.config['AUTO_BACKUP_MINUTE'] = auto_minute

        if days_to_keep is not None:
            app.config['BACKUP_DAYS_TO_KEEP'] = days_to_keep

        logger.info(f"备份设置已更新: 自动备份时间 {auto_hour}:{auto_minute}，保留天数 {days_to_keep}")
...
def api_cleanup_backups():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': f'清理完成，删除 {deleted_count} 个旧备份文件',
